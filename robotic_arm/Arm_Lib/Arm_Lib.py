...
# 兼容 macOS：优先使用 smbus，若不可用则尝试 smbus2，再不行使用空实现以便代码运行
try:
    import smbus  # type: ignore
    _SMBusClass = smbus.SMBus
except ImportError:
    try:
        import smbus2  # type: ignore
        _SMBusClass = smbus2.SMBus
    except ImportError:
        _SMBusClass = None


class _DummySMBus:
    """在无 I2C 总线（如 macOS）时的占位实现，避免导入失败."""
    def __init__(self, bus_num=1):
        self.bus_num = bus_num
        print(f"[Arm_Lib] WARNING: 未找到 I2C 总线，使用 DummySMBus (bus={bus_num})，仅用于开发仿真。")

    def write_i2c_block_data(self, addr, cmd, vals):
        print(f"[Arm_Lib] Dummy write_i2c_block_data addr={addr} cmd={cmd} vals={vals}")

    def write_byte_data(self, addr, cmd, val):
        print(f"[Arm_Lib] Dummy write_byte_data addr={addr} cmd={cmd} val={val}")

    def read_word_data(self, addr, cmd):
        print(f"[Arm_Lib] Dummy read_word_data addr={addr} cmd={cmd}")
        return 0

    def read_byte_data(self, addr, cmd):
        print(f"[Arm_Lib] Dummy read_byte_data addr={addr} cmd={cmd}")
        return 0


def _open_bus(bus_num=1):
    """根据平台尝试打开 I2C，总线不可用时返回 DummySMBus。"""
    if _SMBusClass is None:
        return _DummySMBus(bus_num)
    try:
        return _SMBusClass(bus_num)
    except Exception as e:
        print(f"[Arm_Lib] WARNING: 打开 I2C 总线 {bus_num} 失败（{e}），使用 DummySMBus。")
        return _DummySMBus(bus_num)
...
class Arm_Device(object):
    ...
    def __init__(self):
        self.addr = 0x15
        # macOS 默认无 /dev/i2c 设备，使用兼容打开方式
        self.bus = _open_bus(1)
...
if __name__ == "__main__":
    arm = Arm_Device()
    try:
        v = arm.Arm_get_hardversion()
        print(f"[Arm_Lib Test] 硬件版本: {v}")
    except Exception as e:
        print(f"[Arm_Lib Test] 读取硬件版本失败: {e}")
