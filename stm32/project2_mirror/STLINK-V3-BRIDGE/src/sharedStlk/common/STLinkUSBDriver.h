...
// DLL API versions
// Version 1 (implicit):
//   - Exports only pSTMass_xxxx routines
#define STLINK_LIB_API_VERSION_LEGACY 1

// Version 2 (implicit):
//   - Exports also STLink_xxxx and STLink_xxxxTcp routines
#define STLINK_LIB_API_VERSION_MIN_FOR_TCP 2

// Version 3:
//   - Exports also STLink_GetLibApiVer and STLink_GetServerVersion routines
//   - Maximum command size downstream in TCP mode increased from 2016 to 6144 (at dll level) 
#define STLINK_LIB_API_VERSION_TCP_EXTENDED 3

// Version 4:
//   - Manages ST-Link COM ports
//   - Added interfaceId and PortName in TDeviceInfo2
#define STLINK_LIB_API_VERSION_COM_PORTS 4
...
#define  SS_TCP_CLOSE_ERROR  0x2003  // Error to close the TCP socket (remaining STLink opened ...)
#define  SS_TCP_BUSY         0x2004
...
#define SERIAL_NUM_STR_MAX_LEN 32 ///< Serial number max size

/// COM port name max size
#define STLINK_COM_PORT_STR_MAX_LEN 32

// Constant defining close type in STLink_CloseDeviceTcp() closeTcp parameter
// default, automatically close the TCP socket (and potentially stlink-server) if close of the last remaining opened STLink device.
#define CLOSE_TCP_AUTO   0
// New parameters added to avoid closing the TCP socket at the same time as the last device 
// (if used on old lib until v5.1.2.0 return SS_BAD_PARAMETER)
#define CLOSE_ONLY_DEVICE   1 // Close the STLink device but do not close the TCP socket : need to call later STLink_CloseDeviceTcp(CLOSE_ONLY_TCP)
#define CLOSE_ONLY_TCP   2    // Close the TCP socket (and potentially stlink-server) if no remaining opened STLink device
...
typedef struct {
	uint32_t StLinkUsbId; ///< ST-LINK-SERVER Device cookie in little endian format, to use in STLink_OpenDeviceTcp and STLink_SendCommandTcp
	char EnumUniqueId[SERIAL_NUM_STR_MAX_LEN]; ///< Unique instance ID from system enumeration (equal to
	                                           ///< device Serial number in some cases, but not always ...)
	uint16_t VendorId;  ///< Vendor  ID from USB device descriptor (system enumeration)
	uint16_t ProductId; ///< Product ID from USB device descriptor (system enumeration)
	...
	uint16_t InterfaceId;  ///< Interface ID of the enumerated feature in case of composite device, added in LIB_API v4
	char PortName[STLINK_COM_PORT_STR_MAX_LEN];  ///< Name of COM port if such interface enumerated, added in LIB_API v4
	// NOTE: do not modify the existing fields in the structure. But for any evolution,
	// add a new field at the end in order to keep ascendant compatibility.
} TDeviceInfo2;
...
typedef struct {
	uint32_t ApiVer;   ///< ST-LINK-SERVER API version
	uint32_t MainVer;  ///< ST-LINK-SERVER main version
	uint32_t RevVer;   ///< ST-LINK-SERVER API revision
	uint32_t BuildVer; ///< ST-LINK-SERVER build version
} STLink_ServerVersionT; ///< Structure containing information on ST-LINK-SERVER version

/// #TEnumStlinkInterface: legacy library name, use #STLink_EnumStlinkInterfaceT
typedef enum {
	STLINK_DBG_INTERFACE =0, ///< Interface for ST-Link Debug
	                         // Note that the value is also the interface index in the device (important for implementation on Linux)
	STLINK_RW_INTERFACE,     // Note that the value is also the interface index in the device (important for implementation on Linux)
	STLINK_TCP,              ///< Interface for multi-core Debug through ST-LINK-SERVER
	STLINK_BRIDGE,           ///< Interface for Bridge
	                         // On Linux conversion is done by STLinkLibUSB_ConvToUsbIfId()
	STLINK_COM_PORT_DBG,     ///< Interface for ST-Link COM port Debug
	STLINK_COM_PORT_PWR,     ///< Interface for ST-Link COM port Power
...
#define STLINK_PWR_USB_BRIDGE_IF_NB 3 // Bridge interface index in the device (important for implementation on Linux) on STLink Power
...
///////////////////////////////////////////////////////////////////////////////
// Routines for new API (version > 1)
///////////////////////////////////////////////////////////////////////////////

//******************************************************************************
// STLink_GetLibApiVer:
// Returns the API version of this library, for easier management of compatibility
// in the future. Added into version 3.
// Returns: the API version of this library (starting from value 3; previous versions did not
//          provide this routine)
//******************************************************************************
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t (STD_CALL *pSTLink_GetLibApiVer)(void);
#else
EXPORTED_API uint32_t STD_CALL STLink_GetLibApiVer(void);
#endif
...
//   closeTcp: - only CLOSE_TCP_AUTO until v5.1.2.0 (else return SS_BAD_PARAMETER):
//             Tcp socket is closed if StLinkUsbId was the last opened device.
//             - CLOSE_ONLY_DEVICE: Do not close the TCP socket even if StLinkUsbId was the last opened device.
//             Require to call later STLink_CloseDeviceTcp(CLOSE_ONLY_TCP) to close the TCP socket.
//             - CLOSE_ONLY_TCP &&  StLinkUsbId = 0 : Close the TCP socket (and potentially stlink-server)
//             if no remaining opened device. Else return SS_TCP_CLOSE_ERROR: opened devices must be closed before.
//
// Returns: SS_OK  = SUCCESS, Error otherwise (Error Code)
//******************************************************************************
#ifdef WIN32
...
//******************************************************************************
// STLink_GetServerVersion:
// Returns SS_OK and the TCP server version in pServerVersion parameter, if the
// server has been successfully launched by STLink_ReenumerateTcp
// Otherwise returns SS_TCP_CANT_CONNECT and all fields of pServerVersion are 0
//******************************************************************************
#ifdef WIN32
// Export the signature only; the routine will be linked through GetProcAdddress
typedef uint32_t (STD_CALL *pSTLink_GetServerVersion)(STLink_ServerVersionT *pServerVersion);
#else
EXPORTED_API uint32_t STD_CALL STLink_GetServerVersion(STLink_ServerVersionT *pServerVersion);
#endif
