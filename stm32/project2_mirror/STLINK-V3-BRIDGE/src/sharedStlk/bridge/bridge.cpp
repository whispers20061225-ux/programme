...
bool Brg::IsOldBrgFwVersion(void) const {
	...
	if( ((m_Version.Major_Ver==3) && (m_Version.Bridge_Ver<FIRMWARE_BRIDGE_STLINK_V3_LAST_VERSION)) ||
		((m_Version.Major_Ver==4) && (m_Version.Bridge_Ver<FIRMWARE_BRIDGE_STLINK_V4_LAST_VERSION)) ) {
		return true;
	} else {
		return false;
	}
...
Brg_StatusT Brg::OpenStlink(int StlinkInstId)
{
	...
	ifStatus = StlinkDevice::OpenStlink(StlinkInstId);
...
Brg_StatusT Brg::OpenStlink(const char *pSerialNumber, bool bStrict) {
	...
	ifStatus = StlinkDevice::OpenStlink(pSerialNumber, bStrict, false);
 ...
 *                    #COM_FDCAN (if FDCAN supported by the STLINK)\n
...
Brg_StatusT Brg::CloseBridge(uint8_t BrgCom)
{
	...
	if( (BrgCom != COM_SPI)&&(BrgCom != COM_I2C)&&(BrgCom != COM_CAN)
		&&(BrgCom != COM_FDCAN)&&(BrgCom != COM_GPIO)&&(BrgCom != COM_UNDEF_ALL) ) {
		return BRG_PARAM_ERR;
	} else if ((BrgCom == COM_FDCAN) && (IsFdcanSupport()==false)) {
		// FDCAN not supported on all STLINK or FW Bridge version
...
{
	STLinkIf_StatusT ifStatus = STLINKIF_NO_ERR;
	ifStatus = StlinkDevice::GetVersionExt(pVersion);
											 ...
											 const uint16_t UsbTimeoutMs)
{
	...
	if( brgStat == BRG_TARGET_CMD_ERR ) {
		...
		// to firmware error codes from stlink_fw_const_bridge.h.
...
Brg_StatusT Brg::AnalyzeStatus(const uint16_t *pStatus)
{
	if( pStatus != NULL )
	{
		if( *pStatus != STLINK_BRIDGE_OK )
		{
			...
			if( *pStatus == STLINK_BRIDGE_CMD_NOT_ALLOWED) {
				LogTrace("BRIDGE Command not allowed (e.g.: cannot mix CAN/FDCAN commands)");
				return BRG_CMD_NOT_ALLOWED;
			}
...
Brg_StatusT Brg::GetClk(uint8_t BrgCom, uint32_t *pBrgInputClk, uint32_t *pStlHClk)
{
	...
	if ((BrgCom != COM_SPI)&&(BrgCom != COM_I2C)&&(BrgCom != COM_CAN)&&(BrgCom != COM_GPIO)&&(BrgCom != COM_FDCAN)) {
		return BRG_PARAM_ERR;
	} else if ((BrgCom == COM_FDCAN) && (IsFdcanSupport()==false)) {
		// FDCAN not supported on all STLINK or FW Bridge version
		return BRG_CMD_NOT_SUPPORTED;
 ...
 * In master mode I2C, the size bytes are received in one I2C transaction (start- addr- data -stop)
 ...
 * @retval #BRG_COM_CMD_ORDER_ERR If low level I2C function call order is not consistent (Start, Cont, Stop)
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::ReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t *pSizeRead)
{
	return ReadI2Ccmd(pBuffer, Addr, SizeInBytes, I2C_FULL_RW_TRANS, pSizeRead,
	                  NULL, NULL, 0, I2C_PREP_NONE, I2C_FORCE_NONE);
}
/**
 * @ingroup I2C
 ...
 * @param[out]  pBuffer     See Brg::ReadI2C() above
 ...
 * Below parameters are used only in I2C_FULL_RW_TRANS_WRITE_READ case (NULL or 0 in other case)
 * @param[in] pBufferW pointer on data buffer of WriteSizeInBytes size to be transmitted or NULL if unused
 * @param[in] WriteSizeInBytes 1 to 4 bytes or 0 if unused
 */
Brg_StatusT Brg::ReadI2Ccmd(uint8_t *pBuffer, const uint16_t Addr,
                            const uint16_t SizeInBytes, const Brg_I2cRWTransfer RwTransType,
                            ...
                            const I2cTransPrepT PrepRestart, const I2cTransForceT ForceRestart)
{
	...
	if (((PrepRestart != I2C_PREP_NONE) || (ForceRestart != I2C_FORCE_NONE)) &&  // support starting v3B6 v4B3
	    (IsRestartI2CSupport() == false)) {
		// check I2C_PREP_RESTART for RwTransType= START or CONT not check (manage by calling function)
		// check I2C_FORCE_RESTART for RwTransType= STOP or CONT not check (manage by calling function)
		return BRG_CMD_NOT_SUPPORTED;
	}
	// if WRITE_READ param used verify FW is not too old and 
	// parameters consistency
	if (RwTransType == I2C_FULL_RW_TRANS_WRITE_READ) { // supported starting v3B6 v4B3
		if (IsRestartI2CSupport() == false) {
			return BRG_CMD_NOT_SUPPORTED;
		} else if ((pBufferW == NULL) || (WriteSizeInBytes > 4) || (WriteSizeInBytes < 1)) {
			return BRG_PARAM_ERR;
		} // else OK
	} // pBufferW and WriteSizeInBytes unused otherwise
	...
	if (ForceRestart == I2C_FORCE_RESTART) {
		// Update bit6 of Byte6 to 1
		pRq->CDBByte[6] |= (uint8_t)(1<<6);
	} // else default I2C_FORCE_NONE bit6 = b0
	if (PrepRestart == I2C_PREP_RESTART) {
		// Update bit7 of Byte6 to 1
		pRq->CDBByte[6] |= (uint8_t)(1<<7);
	} // else default I2C_PREP_NONE bit7 = b0
	if (RwTransType == I2C_FULL_RW_TRANS_WRITE_READ) {
		// additionnal bits to be set in pRq->CDBByte[6]
		// Byte6 Bit5:4 Length of the data to write (0: 1byte, 3: 4 bytes)
		pRq->CDBByte[6] |= (uint8_t)((WriteSizeInBytes-1) << 4)&0x30;
		// Byte7 unused (0)
		// Byte8-11 Byte(s) to transfer to target (write 1 to 4 bytes)
		for (i=0; i<WriteSizeInBytes; i++) {
			pRq->CDBByte[8+i] = pBufferW[i];
		}
	}
 ...
 * Master mode I2C transaction is split: only Start, Addr are sent and data are received (no stop sent).
 * @warning After Brg::StartReadI2C() it is mandatory to call Brg::StopReadI2C() (optionally Brg::ContReadI2C()).\n
 *          For function order and parameters usage related to repeated start sequences, see #I2cTransPrepT.
 *
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[out]  pBuffer Pointer on data buffer filled with read data.
 ...
 * @param[out] pSizeRead If not NULL and in case of error, pSizeRead returns the number of bytes
 *             received before the error.
 * @param[in]  PrepRestart Default #I2C_PREP_NONE.\n
 *             #I2C_PREP_RESTART is used in repeated start sequences. See #I2cTransPrepT for parameter usage.\n
 *             #I2C_PREP_RESTART is not supported on old firmware below v3B6 or v4B3.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If I2C is not initialized
 * @retval #BRG_I2C_ERR In case of I2C error
 * @retval #BRG_COM_CMD_ORDER_ERR If low level I2C function call order is not consistent (Start, Cont, Stop)
 * @retval #BRG_CMD_NOT_SUPPORTED If STLINK firmware is too old (see Brg::IsRestartI2CSupport())
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::StartReadI2C(uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes, uint16_t *pSizeRead,
                              const I2cTransPrepT PrepRestart)
{
	Brg_StatusT status;
	m_slaveAddrPartialI2cTrans = Addr;
	status = ReadI2Ccmd(pBuffer, Addr, SizeInBytes, I2C_START_RW_TRANS, pSizeRead,
	                    NULL, NULL, 0, PrepRestart, I2C_FORCE_NONE);
	return status;
}
/**
 * @ingroup I2C
 ...
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 * @param[out]  pBuffer     See Brg::StartReadI2C() above
 * @param[in]  SizeInBytes See Brg::StartReadI2C() above
 * @param[out] pSizeRead   See Brg::StartReadI2C() above
 * @param[in]  PrepRestart See Brg::StartReadI2C() above
 */
Brg_StatusT Brg::StartReadI2C(uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode,
                              uint16_t SizeInBytes, uint16_t *pSizeRead, const I2cTransPrepT PrepRestart)
{
	 uint16_t slaveAddr = Addr; // default bit15=0 7b
	 if( AddrMode == I2C_ADDR_10BIT ) {
		slaveAddr = I2C_10B_ADDR(Addr); // set bit15 to 1 for 10b
	 }
	 return StartReadI2C(pBuffer, slaveAddr, SizeInBytes, pSizeRead, PrepRestart);
}
/**
 * @ingroup I2C
 * @brief This low level routine allows to split I2C transaction and to receive bytes on the I2C interface,
 * in the mode initialized by Brg::InitI2C().\n
 * For a full I2C transaction use Brg::ReadI2C() instead.\n
 * Master mode I2C transaction is split: by default only data are received (no start-address or stop sent).\n
 * A repeated start may be generated in sequences detailed in #I2cTransPrepT description.
 ...
 *             #I2C_PREP_RESTART is used in repeated start sequences. See #I2cTransPrepT for parameter usage.\n
 *             #I2C_PREP_RESTART is not supported on old firmware below v3B6 or v4B3.
 * @param[in]  ForceRestart Default #I2C_FORCE_NONE.\n
 *             #I2C_FORCE_RESTART is used in repeated start sequences. See #I2cTransPrepT for parameter usage.\n
 *             #I2C_FORCE_RESTART is not supported on old firmware below v3B6 or v4B3.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If I2C is not initialized
 * @retval #BRG_I2C_ERR In case of I2C error
...
{
	Brg_StatusT status;
	Brg_I2cRWTransfer i2ctrans = I2C_CONT_RW_TRANS;
	status = ReadI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes, i2ctrans, pSizeRead,
	                    NULL, NULL, 0, PrepRestart, ForceRestart);
	return status;
}
/**
 * @ingroup I2C
 ...
 * in the mode initialized by Brg::InitI2C().\n
 * For a full I2C transaction use Brg::ReadI2C() instead.\n
 * Master mode I2C transaction is split: only data are received and stop sent (no start-address sent).\n
 * A repeated start may be generated in sequences detailed in #I2cTransPrepT description.
...
Brg_StatusT Brg::StopReadI2C(uint8_t *pBuffer, uint16_t SizeInBytes, uint16_t *pSizeRead,
                             const I2cTransForceT ForceRestart)
{
	Brg_StatusT status;
	status = ReadI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes, I2C_STOP_RW_TRANS, pSizeRead,
	                    NULL, NULL, 0, I2C_PREP_NONE, ForceRestart);
	return status;
}
/**
 * @ingroup I2C
 ...
 * Main use case is for I2C transaction requiring timeout higher than 200ms to complete.
...
/**
 * @ingroup I2C
 * @brief This routine allows to do a Write (max 4bytes) Read transaction on the I2C interface,
 * with a repeated start between the Write and the Read, in the mode initialized by Brg::InitI2C().\n
 * In master mode I2C, the WriteSizeInBytes bytes are sent then ReadSizeInBytes are received in one
 * I2C transaction with a RESTART between sent and received data (Start- addr- Wdata- restart-addr- Rdata -stop)
 * even if data may be split into several USB packets. \n
 * If more than 4 write data bytes to be sent, Write Read repeated start is supported by splitting the transaction
 * using low-level routines: Brg::StartWriteI2C(#I2C_PREP_RESTART) followed by Brg::StopReadI2C().\n
 * Using Brg::WriteReadI2C() gives better performance than calling Brg::StartWriteI2C() and Brg::StopReadI2C().\n
 * @warning This routine is not supported on old firmware below v3B6 or v4B3.\n
 *          This routine may potentially slow down the other USB interfaces (VCP, Debug) in case
 *          of slave slowness during the write phase (long slave clock stretching).
 *          If needed to avoid this, use the low-level routines instead.
 *
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[out] pBufferR Pointer on data buffer filled with read data.
 * @param[in]  ReadSizeInBytes Data size to be read in bytes (min 1, max data bufferR size)
 * @param[out] pSizeRead If not NULL and in case of error, pSizeRead returns the number of bytes
 *             received before the error.
 * @param[in]  pBufferW Pointer on data buffer with data to be sent.
 * @param[in]  WriteSizeInBytes Data size to be sent in bytes (min 1, max 4 must not exceed bufferW size)
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_COM_INIT_NOT_DONE If I2C is not initialized
 * @retval #BRG_I2C_ERR In case of I2C error
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::WriteReadI2C(uint8_t *pBufferR, const uint16_t Addr, const uint16_t ReadSizeInBytes, uint16_t *pSizeRead,
							  const uint8_t *pBufferW,  const uint16_t WriteSizeInBytes)
{
	return ReadI2Ccmd(pBufferR, Addr, ReadSizeInBytes, I2C_FULL_RW_TRANS_WRITE_READ, pSizeRead, NULL,
	                  pBufferW, WriteSizeInBytes, I2C_PREP_NONE, I2C_FORCE_NONE);
}
/**
 * @ingroup I2C
 * @brief Same as WriteReadI2C(uint8_t *pBufferR, const uint16_t Addr, const uint16_t ReadSizeInBytes,
 *							   uint16_t *pSizeRead, const uint8_t *pBufferW,  const uint16_t WriteSizeInBytes)
 *        except for Addr parameter
 * @param[in]  Addr Is the I2C slave address used in master mode.
 * @param[in]  AddrMode Indicate if it is a 10bit or 7bit address.
 * @param[out] pBufferR         See Brg::WriteReadI2C() above
 * @param[in]  ReadSizeInBytes  See Brg::WriteReadI2C() above
 * @param[out] pSizeRead        See Brg::WriteReadI2C() above
 * @param[in]  pBufferW         See Brg::WriteReadI2C() above
 * @param[in]  WriteSizeInBytes See Brg::WriteReadI2C() above
 */
Brg_StatusT Brg::WriteReadI2C(uint8_t *pBufferR, const uint16_t Addr, const Brg_I2cAddrModeT AddrMode,
							  ...
							  const uint8_t *pBufferW,  const uint16_t WriteSizeInBytes)
{
	 ...
	 return WriteReadI2C(pBufferR, slaveAddr, ReadSizeInBytes, pSizeRead, pBufferW, WriteSizeInBytes);
}
 ...
 * In master mode I2C the size bytes are transmitted in one I2C transaction (start- addr- data -stop)
 * even if data may be split into several USB packets.
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 * @param[in]  pBuffer Pointer on data buffer with data to be sent.
 ...
 * RwTransType gives the type of I2C transaction to perform
 * along with PrepRestart and ForceRestart.
 * - if not NULL, pErrorInfo currently unused
 */
                             ...
                             const I2cTransPrepT PrepRestart, const I2cTransForceT ForceRestart)
{
	...
	if (((PrepRestart != I2C_PREP_NONE) || (ForceRestart != I2C_FORCE_NONE)) &&  // supported starting v3B6 v4B3
	    (IsRestartI2CSupport() == false)) {
		// check I2C_PREP_RESTART for RwTransType= START or CONT not check (manage by calling function)
		// check I2C_FORCE_RESTART for RwTransType= STOP or CONT not check (manage by calling function)
		return BRG_CMD_NOT_SUPPORTED;
	...
	pRq->CDBByte[4] = (uint8_t)Addr;
	pRq->CDBByte[5] = (uint8_t)(Addr>>8);
	// Byte6 Bit3:0 I2C transaction type:
	pRq->CDBByte[6] = (uint8_t) (RwTransType & I2C_TRANS_TYPE_MSK);
	if (ForceRestart == I2C_FORCE_RESTART) {
		// Update bit6 of Byte6 to 1 force Restart
		pRq->CDBByte[6] |= (uint8_t)(1<<6);
	} // else default I2C_FORCE_NONE bit6 = b0
	if (PrepRestart == I2C_PREP_RESTART) {
		// Update bit7 of Byte6 to 1 prepare Restart
		pRq->CDBByte[6] |= (uint8_t)(1<<7);
	} // else default I2C_PREP_NONE bit7 = b0
 ...
 * Master mode I2C transaction is split: only start- addr- data are sent (no stop).
 * @warning After Brg::StartWriteI2C() it is mandatory to call Brg::StopWriteI2C() (optionally Brg::ContWriteI2C()).\n
 *          For function order and parameters usage related to repeated start sequences, see #I2cTransPrepT.
 *
 * @param[in]  Addr  I2C slave address used in master mode (default 7bit):
 *             use #I2C_10B_ADDR(Addr) if it is a 10bit address.
 ...
 * @retval #BRG_CMD_NOT_SUPPORTED If STLINK firmware is too old (see Brg::IsRestartI2CSupport())
 */
Brg_StatusT Brg::StartWriteI2C(const uint8_t *pBuffer, uint16_t Addr, uint16_t SizeInBytes,
                               uint16_t *pSizeWritten, const I2cTransPrepT PrepRestart)
{
	Brg_StatusT status;
	m_slaveAddrPartialI2cTrans = Addr;
	status = WriteI2Ccmd(pBuffer, Addr, SizeInBytes, I2C_START_RW_TRANS, pSizeWritten,
	                     NULL, PrepRestart, I2C_FORCE_NONE);
	return status;
}
/**
 * @ingroup I2C
 ...
 * @param[in]  PrepRestart  See Brg::StartWriteI2C() above
 */
Brg_StatusT Brg::StartWriteI2C(const uint8_t *pBuffer, uint16_t Addr, Brg_I2cAddrModeT AddrMode,
                            uint16_t SizeInBytes, uint16_t *pSizeWritten, const I2cTransPrepT PrepRestart)
{
	 uint16_t slaveAddr = Addr; // default bit15=0 7b
	 if( AddrMode == I2C_ADDR_10BIT ) {
		slaveAddr = I2C_10B_ADDR(Addr); // set bit15 to 1 for 10b
	 }
	 return StartWriteI2C(pBuffer, slaveAddr, SizeInBytes, pSizeWritten, PrepRestart);
}
/**
 * @ingroup I2C
 * @brief This low level routine allows to split I2C transaction and to send bytes on the I2C interface,
...
{
	Brg_StatusT status;
	status = WriteI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes, I2C_CONT_RW_TRANS, pSizeWritten,
	                     NULL, PrepRestart, ForceRestart);
	return status;
}
/**
 * @ingroup I2C
 ...
 * in the mode initialized by Brg::InitI2C().\n
 * For a full I2C transaction use Brg::WriteI2C() instead.\n
 * Master mode I2C transaction is split: only data and stop are sent (no start-address).\n
 * A repeated start may be generated in sequences detailed in #I2cTransPrepT description.
...
{
	Brg_StatusT status;
	status = WriteI2Ccmd(pBuffer, m_slaveAddrPartialI2cTrans, SizeInBytes, I2C_STOP_RW_TRANS, pSizeWritten,
	                     NULL, I2C_PREP_NONE, ForceRestart);
	return status;
}

// -------------------------------- CAN ------------------------------------ //
//  private function used to verify Brg_CanBitTimeConfT fields for CAN or FDCAN in Classic CAN
Brg_StatusT Brg::CheckBitTimeClassicCAN(const Brg_CanBitTimeConfT* pBitTimeConf)
{
 ...
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_PARAM_ERR Null pointer parameter
 * @retval BRG_CMD_NOT_ALLOWED if CAN com is already initialized (CAN and FDCAN are exclusive, close CAN com and retry)
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::InitCAN(const Brg_CanInitT *pInitParams, Brg_InitTypeT InitType)
{
	...
	brgStat = CheckBitTimeClassicCAN(pBitTimeConf);
	if( brgStat != BRG_NO_ERR  ) {
		return brgStat;
	}
	// Check prescaler
 ...
 *                       else if found prescaler is >1024 baudrate or N need to be increased.\n
 *    Tq = time quanta \n
 ...
 * 32bit Standard ID: [31:21] = Id[10:0], [20:3]=0, [2]=IDE (=0), [1]=RTR (=0), [0]=0
 * 32bit Extended ID: [31:3] = Id[28:0], [2]=IDE, [1]=RTR, [0]=0
...
Brg_StatusT Brg::FormatFilter32bitCAN(const Brg_FilterBitsT *pInConf, uint8_t *pOutConf)
{
	...
	if( pInConf->IDE == CAN_ID_EXTENDED ) {
		uint32_t tmpID = (pInConf->ID<<3)&0xFFFFFFF8;
		pOutConf[0] |= (uint8_t)(tmpID&0xFF); // [7:3]= Id[4:0]
		pOutConf[1] |= (uint8_t)((tmpID>>8)&0xFF); // [15:8]= Id[12:5]
		pOutConf[2] |= (uint8_t)((tmpID>>16)&0xFF); // [23:16] = Id[20:13]
		pOutConf[3] |= (uint8_t)((tmpID>>24)&0xFF); // [31:24] = Id[29:21]
	} else { // CAN_ID_STANDARD
...
Brg_StatusT Brg::InitFilterCAN(const Brg_CanFilterConfT *pInitParams)
{
	...
	// Check filter configuration supported by the firmware
	if( IsCanFilter16Support() == false ) {
		// 16bit filter and mixed Standard and extended ID not supported
		if( (pInitParams->FilterScale == CAN_FILTER_16BIT) ||
			((pInitParams->FilterMode == CAN_FILTER_ID_LIST) && (pInitParams->Id[0].IDE != pInitParams->Id[1].IDE)) ) {
			return BRG_NOT_SUPPORTED;
		}
	}
...
// -------------------------------- FDCAN ----------------------------------- //
//  private function used to verify Brg_CanBitTimeConfT fields for FDCAN
Brg_StatusT Brg::CheckBitTimeFDCAN(const Brg_CanBitTimeConfT* pBitTimeConf, const Brg_FdcanFrameModeT CanMode, const bool bIsNomBitTime)
{
	// Check BitTimeConf parameters
	if (CanMode == FDCAN_FRAME_CLASSIC) {
		return CheckBitTimeClassicCAN(pBitTimeConf);
	} else { // FDCAN_FRAME_FD_NO_BRS or FDCAN_FRAME_FD_BRS
			 // Check STLINK-V3PWR FDCAN H7 Nominal timing max values
		if ( (bIsNomBitTime == true) &&
		     ((pBitTimeConf->PropSegInTq < 1) || ((int)pBitTimeConf->PropSegInTq > 256) || // PROP_SEG = 1 to 256 time quantum;
		       // explicit cast to int added to remove warning while keeping the test against 256 for robustness in case of future type evolution
		     (pBitTimeConf->PhaseSeg1InTq < 1) || ((int)pBitTimeConf->PhaseSeg1InTq > 256) || // PHASE_SEG1 = 1 to 256 time quantum
		     ((pBitTimeConf->PropSegInTq + pBitTimeConf->PhaseSeg1InTq) > 256) || // BS1 =PROP_SEG + PHASE_SEG1  BS1 max is 256 time quantum
		     (pBitTimeConf->PhaseSeg2InTq < 1) || (pBitTimeConf->PhaseSeg2InTq > 128) || // PHASE_SEG2 = 1 to 128 time quantum
		     (pBitTimeConf->SjwInTq < 1) || (pBitTimeConf->SjwInTq > 128) // Synchro Jump Width = 1 to 128 time quantum
		     )) {
			return BRG_PARAM_ERR;
		}
		// Check STLINK-V3PWR FDCAN H7 Data timing max values
		if ((bIsNomBitTime == false) &&
		    ((pBitTimeConf->PropSegInTq < 1) || (pBitTimeConf->PropSegInTq > 32) || // PROP_SEG = 1 to 32 time quantum
		    (pBitTimeConf->PhaseSeg1InTq < 1) || (pBitTimeConf->PhaseSeg1InTq > 32) || // PHASE_SEG1 = 1 to 32 time quantum
		    ((pBitTimeConf->PropSegInTq + pBitTimeConf->PhaseSeg1InTq) > 32) || // BS1 =PROP_SEG + PHASE_SEG1  BS1 max is 32 time quantum
		    (pBitTimeConf->PhaseSeg2InTq < 1) || (pBitTimeConf->PhaseSeg2InTq > 16) || // PHASE_SEG2 = 1 to 16 time quantum
		    (pBitTimeConf->SjwInTq < 1) || (pBitTimeConf->SjwInTq > 16) // Synchro Jump Width = 1 to 16 time quantum
		    )) {
			return BRG_PARAM_ERR;
		}
	}
...
/*
 * private function used by  Brg::InitFDCAN()
 * @brief This routine checks and sends bitTime FDCAN commands,
 *         mandatory before sending STLINK_BRIDGE_INIT_FDCAN\n
 * @param[in]  pBitTimeConf Nominal or Data BitTime parameter
 * @param[in]  Prescaler Nominal or Data prescaler parameter
 * @param[in]  CanMode Classic CAN or FD CAN with or without BRS.
 * @param[in]  bIsNomBitTime true for Nominal BitTime parameter and false for Data  BitTime parameter.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_PARAM_ERR Bad timing parameters.
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::InitBitTimeFDCAN(const Brg_CanBitTimeConfT* pBitTimeConf, const uint32_t Prescaler, const Brg_FdcanFrameModeT FrameMode, const bool bIsNomBitTime)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint16_t status;
	...
	if (IsFdcanSupport() == false) {
		// FDCAN not supported on all STLINK or FW Bridge version
		return BRG_CMD_NOT_SUPPORTED;
	}
	if (pBitTimeConf == NULL) {
		return BRG_PARAM_ERR;
	}
	// Check BitTimeConf parameters
	brgStat = CheckBitTimeFDCAN(pBitTimeConf, FrameMode, bIsNomBitTime);
	if (brgStat != BRG_NO_ERR) {
		return brgStat;
	}
	// Check STLINK-V3PWR FDCAN H7 Nominal timing max values
	if ( (bIsNomBitTime == true) &&	((Prescaler < 1) || (Prescaler > 512))) {
		return BRG_PARAM_ERR;
	}
	// Check STLINK-V3PWR FDCAN H7 Data timing max values
	if ((bIsNomBitTime == false) && ((Prescaler < 1) || (Prescaler > 32))) {
		return BRG_PARAM_ERR;
	}
	...
	pRq->CDBLength = STLINK_BRIDGE_CMD_SIZE_16;
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	// send the correct command according if Nominal or Data Timing parameters
	if (bIsNomBitTime == true) {
		pRq->CDBByte[1] = STLINK_BRIDGE_INIT_NBITTIME_FDCAN;
	}
	else {
		pRq->CDBByte[1] = STLINK_BRIDGE_INIT_DBITTIME_FDCAN;
	}
	// BitTiming parameters -1 because 1_time_quantum = 0
	// Nominal Bit Time propagation (PROP_SEG)
	pRq->CDBByte[2] = pBitTimeConf->PropSegInTq-1;
	// Nominal Bit Time phase segment1 (PHASE_SEG1)
	pRq->CDBByte[3] = pBitTimeConf->PhaseSeg1InTq-1;
	// Nominal Bit Time phase segment2(PHASE_SEG2)
	pRq->CDBByte[4] = pBitTimeConf->PhaseSeg2InTq-1;
	// Nominal Bit Time Synchronisation Jump Width (SJW)
	pRq->CDBByte[5] = pBitTimeConf->SjwInTq-1;
	// BaudRate Prescaler
	pRq->CDBByte[6] = (uint8_t)(Prescaler) & 0xFF;
	pRq->CDBByte[7] = (uint8_t)(Prescaler >> 8) & 0xFF;
...
/**
 * @ingroup FDCAN
 * @brief This routine initializes the FDCAN according to init parameters.\n
 * Refer to STLink firmware upgrade release note (stsw-link007) for supported init configurations.\n
 * Call Brg::GetFDCANbaudratePrescal() to get prescaler parameter according to required FDCAN bitrate.
 * @param[in]  pInitParams FDCAN initialization parameters see #Brg_FdcanInitT and \ref LIMITATION
 * @param[in]  InitType  #BRG_INIT_FULL for normal case (CLK, reset, Filter deactivated) or \n
 *                       #BRG_REINIT for configuration change only.
 * @param[in]  bStartBus true (default): call Brg::StartFDCAN() to enter normal bus operation,
 *                       false Brg::StartFDCAN() need to be called later outside this function
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_PARAM_ERR Null pointer parameter or bad parameter configuration.
 * @retval BRG_CMD_NOT_ALLOWED if CAN com is already initialized (CAN and FDCAN are exclusive, close CAN com and retry)
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::InitFDCAN(const Brg_FdcanInitT* pInitParams, Brg_InitTypeT InitType, bool bStartBus)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint16_t status;
	bool bIsNomBitTime = true; // default Nominal timing
	uint8_t conf;
	...
	if ((pInitParams->bIsTdcEn == true) && ((pInitParams->tdcOffset>127)||(pInitParams->tdcFilter>127))) {
		return BRG_PARAM_ERR;
	}

	// First send Nominal and Data Timing parameters commands (required before STLINK_BRIDGE_INIT_FDCAN)
	bIsNomBitTime = true; // Send Nominal Timing parameters
	brgStat = InitBitTimeFDCAN(&pInitParams->NomBitTimeConf, pInitParams->NomPrescaler, pInitParams->FrameMode, bIsNomBitTime);
	if (brgStat == BRG_NO_ERR) {
		bIsNomBitTime = false; // Send Data Timing parameters
		// FDCAN_FRAME_CLASSIC FDCAN_FRAME_FD_NO_BRS does not really use data bit rate switching
		brgStat = InitBitTimeFDCAN(&pInitParams->DataBitTimeConf, pInitParams->DataPrescaler, pInitParams->FrameMode, bIsNomBitTime);
	}

	if (brgStat == BRG_NO_ERR) {
		// STLINK_BRIDGE_INIT_NBITTIME_FDCAN and STLINK_BRIDGE_INIT_DBITTIME_FDCAN have been sent successfully.
		// Continue with STLINK_BRIDGE_INIT_FDCAN.
		pRq = new STLink_DeviceRequestT;
		memset(pRq, 0, sizeof(STLink_DeviceRequestT));

		pRq->CDBLength = STLINK_BRIDGE_CMD_SIZE_16;
		pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
		pRq->CDBByte[1] = STLINK_BRIDGE_INIT_FDCAN;
		// Can Mode
		pRq->CDBByte[2] = (uint8_t)pInitParams->Mode;
		// Init Type
		pRq->CDBByte[3] = (uint8_t)InitType;
		// Frame Mode
		pRq->CDBByte[4] = (uint8_t)pInitParams->FrameMode;
		// Configuration
		// Bit0 DisAr : Enable automatic retransmission mode (0 default) or disable automatic retransmission mode (1)
		// Bit1 Txp : Disable (0 default) the Transmit Pause feature or Enable TxP (1)
		// Bit2 DisPxh : Enable the Protocol exception handling (0 default) or disable the Protocol exception handling (1)
		// Bit3 fifo0Mode : blocking (0 default)  overwrite (1)
		// Bit4 fifo1Mode : blocking (0 default)  overwrite (1)
		// Bit7-5 unused (0)
		conf = 0;
		if (pInitParams->bIsArEn == false) {
			conf |= 1 << 1;
		}
		if (pInitParams->bIsTxpEn == true) {
			conf |= 1 << 2;
		}
		if (pInitParams->bIsPexhEn == false) {
			conf |= 1 << 3;
		}
		if (pInitParams->Fifo0Mode == FDCAN_FIFO_OVERWRITE) {
			conf |= 1 << 4;
		}
		if (pInitParams->Fifo1Mode == FDCAN_FIFO_OVERWRITE) {
			conf |= 1 << 5;
		}
		pRq->CDBByte[5] = conf;
		if (pInitParams->bIsTdcEn == true) {
			// byte6 Transmitter Delay compensation offset (to be used for high baudrate)
			// Bit 7: 0 disable (default) 1 enable
			// Bit0-6: 0-127
			pRq->CDBByte[6] = (uint8_t)(1 << 7) | (pInitParams->tdcOffset & 0x7F);
			// byte7 Transmitter Delay compensation filter window length
			// Bit 7: 0 disable (default) 1 enable
			// Bit0-6: 0-127: 
			pRq->CDBByte[7] = (uint8_t)(1 << 7) | (pInitParams->tdcFilter & 0x7F);
		}
	...
	if ((brgStat == BRG_NO_ERR) && (bStartBus==true)) {
		// Finalize initialisation by connecting to the BUS
		// Send STLINK_BRIDGE_START_FDCAN command.
		brgStat = StartFDCAN();
	}
...
/**
* @ingroup FDCAN
* @brief This routine is called inside Brg::InitFDCAN() to finish the initialisation phase
*        and enter normal bus operations. This routine may be used after having called
*        Brg::StopFDCAN() to enter back normal bus operation.\n
*        Brg::StopFDCAN() can be used later to enter back initialisation phase.
*        Brg::StartFDCAN() must be called after to connect again to the bus.\n
*        Compared to CAN API, in FDCAN API Init and Start have been split
*        to allow additionnal initialization to be added in the future if needed.
*
* @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
* @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
* @retval #BRG_COM_INIT_NOT_DONE If FDCAN is not initialized
* @retval #BRG_CAN_ERR In case of FDCAN error
* @retval #BRG_NO_ERR If no error
*/
Brg_StatusT Brg::StartFDCAN(void)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint16_t status;
	...
	pRq->CDBLength = STLINK_BRIDGE_CMD_SIZE_16;
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_START_FDCAN;
...
/**
* @ingroup FDCAN
* @brief This routine can be called after Brg::InitFDCAN() to enter back initialization
*        and stop normal bus operations.
*        Then Brg::StartFDCAN() or Brg::InitFDCAN() must be called again before trying
...
* @retval #BRG_NO_ERR If no error
*/
Brg_StatusT Brg::StopFDCAN(void)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint16_t status;
	...
	pRq->CDBLength = STLINK_BRIDGE_CMD_SIZE_16;
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_STOP_FDCAN;
...
/**
* @ingroup FDCAN
* @brief This routine gets the FDCAN  prescaler needed for Brg::InitFDCAN() according to required FDCAN bitrate.\n
* Refer to STLink firmware upgrade release note (stsw-link007) for supported init configurations.\n
* On FDCAN interface 2 timings configurations are used, nominal and data configuration. 
* In CAN FD fast frames (BRS ON), the bit timing is switched inside the frame, after the BRS (bitrate
* switch) bit, if this bit is recessive. Before the BRS bit, in the CAN FD arbitration phase, the
* nominal CAN bit timing is used as defined by NomBitTimeConf and NomPrescaler in Brg_FdcanInitT.
* In the following CAN FD data phase, the fast CAN bit timing is used as
* definedby DataBitTimeConf and DataPrescaler in Brg_FdcanInitT The bit timing is
* switched back from the fast timing at the CRC delimiter or when an error is detected.\n
* In Classic CAN or when BRS is OFF, only nominal bit timing configuration is used to transmit the frame.\n
* The ratio between the Nominal and Data baudrate should be such that the nodes can reliably detect the switch.
* This often means that requested data baudrate should be an integer multiple of Nominal baudrate.
* 
* @param[in]  pBitTimeConf Nominal or Data FDCAN Bit Time configuration that will also be used in Brg::FDCAN_Init().
* @param[in]  ReqBaudrate  Nominal or Data requested baudrate: max 1000000 1Mbps for Nominal baudrate and 8000000 8Mbps for Data baudrate.
* @param[in]  FrameMode    Frame mode initialised in #Brg_FdcanInitT (default should be #FDCAN_FRAME_FD_BRS)
* @param[in]  bIsNomBitTime Nominal (true) or Data (false) configuration used for pBitTimeConf and pPrescal.
*
* @param[out] pPrescal  Nominal or Data prescaler to be used in Brg::InitFDCAN() parameters for the given bitrate and
*                       bit Time configuration.\n
*                       If found prescaler is <1 baudrate or N need to be decreased,
*                       else if found prescaler is >512 or >32 baudrate or N need to be increased.\n
*    Tq = time quanta \n
*    BaudRate = 1/BitTime \n
*    <table>
*    <tr><td>                <td colspan="4"> Bit Time
*    <tr><td>                <td>              <td colspan="2">Tbs1                <td>Tbs2
*    <tr><td>                <td>sync_seg      <td>prop_seg      <td>phase_seg1    <td>phase_seg2
*    <tr><td>  Classic CAN   <td>1xTq          <td>1 to 8 xTq    <td>1 to 8 xTq    <td>1 to 8 xTq
*    <tr><td>  Nominal FDCAN <td>1xTq          <td colspan="2"> 1 to 256 xTq       <td>1 to 128 xTq
*    <tr><td>  Data FDCAN    <td>1xTq          <td colspan="2"> 1 to 32 xTq        <td>1 to 16 xTq
*    </table>
*    BitTime = Tsync_seg + Tprop_seg + Tphase_seg1 + Tphase_seg2 \n
*            = N x Tq \n
*    Tq = Prescaler x TfdcanClk \n
*    Prescaler = 1/ (N x BaudRate x TfdcanClk) = FfdcanClk / (N x BaudRate) \n
* @param[out] pFinalBaudrate  Found baudRate if #BRG_COM_FREQ_MODIFIED
*
* @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
* @retval #BRG_PARAM_ERR If null pointer parameter or incorrect #Brg_CanBitTimeConfT
* @retval #BRG_COM_FREQ_MODIFIED If ReqBaudrate is different from pFinalBaudrate
* @retval #BRG_COM_FREQ_NOT_SUPPORTED If found prescaler is not in the correct range (Nominal:1-512 or Data:1-32)
* @retval #BRG_NO_ERR If no error
*/
Brg_StatusT Brg::GetFDCANbaudratePrescal(const Brg_CanBitTimeConfT* pBitTimeConf, uint32_t ReqBaudrate,
	uint32_t* pPrescal, uint32_t* pFinalBaudrate, const Brg_FdcanFrameModeT FrameMode, const bool bIsNomBitTime)
{
	Brg_StatusT brgStat;
	uint32_t canInputClkKHz=0;
	uint32_t stlHClkKHz=0;
	...
	// Check requested BaudRate (max 1Mb/s for nominal and 8Mb/s for data)
	if ((ReqBaudrate < 1) || ((bIsNomBitTime==true)&&(ReqBaudrate > 1000000)) ||
	    ((bIsNomBitTime==false)&&(ReqBaudrate > 8000000))) {
		return BRG_PARAM_ERR;
	}
	// Get the current FDCAN input Clk
	brgStat = GetClk(COM_FDCAN, &canInputClkKHz, &stlHClkKHz);
	if (brgStat == BRG_NO_ERR) {
		/* Bit timing
		The bit timing logic monitors the serial bus-line and performs sampling and adjustment of
		the sample point by synchronizing on the start-bit edge and resynchronizing on the following edges.
		...
		The resynchronization Jump Width (SJW) defines an upper bound to the amount of
		lengthening or shortening of the bit segments.
		Tq = time quanta
		BaudRate = 1/NominalBitTime
		<-------------------------------- Nominal bit Time ------------------------------------->
		| sync_seg |       prop_seg        |        phase_seg1        |       phase_seg2        |
		<-- 1xTq --><- 1 to 8/256/32 x Tq -><-- 1 to 8/256/32 x Tq  --><-- 1 to 8/128/16 x Tq -->
		            <---------------------- Tbs1 ---------------------><--------- Tbs2 --------->
		NominalBitTime = Tsync_seg + Tprop_seg + Tphase_seg1 + Tphase_seg2
		= 1xTq + Tbs1 +Tbs2
		= NxTq
		...
		Prescaler = 1/ (N x BaudRate x TfdcanClk) = FfdcanClk / (N x BaudRate)
		*/
		// Calculate Prescaler = 1/ (N x BaudRate x TfdcanClk) = FfdcanClk / (N x BaudRate)
		// N = 1 + PROP_SEG + PHASE_SEG1 + PHASE_SEG2
		n = 1 + pBitTimeConf->PropSegInTq + pBitTimeConf->PhaseSeg1InTq + pBitTimeConf->PhaseSeg2InTq;
		calcPrescal = canInputClkKHz * 1000 / (n * ReqBaudrate);
		if (calcPrescal == 0) {
		...
		*pFinalBaudrate = canInputClkKHz * 1000 / (n * calcPrescal);
		...
		// Nominal prescaler must be between 1 and 512
		// Data prescaler must be between 1 and 32
		if ((calcPrescal < 1)|| ((bIsNomBitTime==true)&&(calcPrescal > 512)) ||
			((bIsNomBitTime==false)&&(calcPrescal > 32))) {
			// if found prescaler is <1 baudrate or N need to be decreased,
			// if found prescaler is >512 or >32 baudrate or N need to be increased.
			brgStat = BRG_COM_FREQ_NOT_SUPPORTED;
		} else if (ReqBaudrate != *pFinalBaudrate) {
			brgStat = BRG_COM_FREQ_MODIFIED; // ReqBaudrate is different from pFinalBaudrate
		}
...
/**
 * @ingroup FDCAN
 * @brief This routine initializes the FDCAN Filters according to init parameters.\n
 * Up to 28 standard filters and 8 extended filters can be configured.
 * @param[in]  pInitParams FDCAN filter configuration see #Brg_FdcanFilterConfT for details.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_COM_INIT_NOT_DONE If FDCAN is not initialized
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::InitFilterFDCAN(const Brg_FdcanFilterConfT* pInitParams)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint16_t status;
	...
	// Check Filter number
	if (((pInitParams->IDE == CAN_ID_STANDARD) && (pInitParams->FilterNb > 27)) ||
		((pInitParams->IDE == CAN_ID_EXTENDED) && (pInitParams->FilterNb > 7))) {
		return BRG_PARAM_ERR;
	}
	// Check filter ID
	if (((pInitParams->IDE == CAN_ID_EXTENDED) && ((pInitParams->ID1 > 0x1FFFFFFF) || (pInitParams->ID2 > 0x1FFFFFFF))) ||
		((pInitParams->IDE == CAN_ID_STANDARD) && ((pInitParams->ID1 > 0x7FF) || (pInitParams->ID2 > 0x7FF))) ||
		((pInitParams->FilterMode == FDCAN_FILTER_ID_RANGE) && (pInitParams->ID1 > pInitParams->ID2))) {
		return BRG_PARAM_ERR;
	}
	pRq = new STLink_DeviceRequestT;
	memset(pRq, 0, sizeof(STLink_DeviceRequestT));
	...
	pRq->CDBLength = STLINK_BRIDGE_CMD_SIZE_16;
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_INIT_FILTER_FDCAN;

	// Filter configuration
	// Bit0 Filter Id type FDCAN_Filter_StdId(0) FDCAN_Filter_ExtId(1)
	// Bit2-1 Filter type FDCAN_FilterType_Range(b00) FDCAN_FilterType_Id(b01) FDCAN_FilterType_Mask(b10)
	// Bit7-3 Filter Number: standard or extended filter to configure
	tmp = 0;
	if (pInitParams->IDE == CAN_ID_EXTENDED) {
		tmp |= 1;
	}//else CAN_ID_STANDARD (0)
	if (pInitParams->FilterMode == FDCAN_FILTER_ID_LIST) {
		tmp |= 1 << 1;
	} else if (pInitParams->FilterMode == FDCAN_FILTER_ID_MASK) {
		tmp |= 2 << 1;
	} // else FDCAN_FILTER_ID_RANGE (0)
	tmp |= pInitParams->FilterNb << 3;
	pRq->CDBByte[2] = tmp;
	// Filter type (check priority order is important to choose correct filter type to configure)
	if (pInitParams->bIsFilterEn == false) {
		tmp = 0; // FDCAN_Filter_Disable (0)
	} else if (pInitParams->bIsFilterReject == true) {
		tmp = 3; // FDCAN_Filter_Reject (3)
	} else if (pInitParams->AssignedFifo == CAN_MSG_RX_FIFO1) {
		tmp = 2; // FDCAN_Filter_FIFO1 (2)
	} else { // CAN_MSG_RX_FIFO0
		tmp = 1; // FDCAN_Filter_FIFO0 (1)
	}
	pRq->CDBByte[3] = tmp;
	pRq->CDBByte[4] = (uint8_t)(pInitParams->ID1);
	pRq->CDBByte[5] = (uint8_t)(pInitParams->ID1 >> 8);
	pRq->CDBByte[6] = (uint8_t)(pInitParams->ID1 >> 16);
	pRq->CDBByte[7] = (uint8_t)(pInitParams->ID1 >> 24);
	pRq->CDBByte[8] = (uint8_t)(pInitParams->ID2);
	pRq->CDBByte[9] = (uint8_t)(pInitParams->ID2 >> 8);
	pRq->CDBByte[10] = (uint8_t)(pInitParams->ID2 >> 16);
	pRq->CDBByte[11] = (uint8_t)(pInitParams->ID2 >> 24);
...
/**
 * @ingroup FDCAN
 * @brief This routine activates the FDCAN RX messages reception storage at STLink side.\n
 * Brg::StartMsgReceptionFDCAN() must be called before Brg::GetRxMsgNbFDCAN() and Brg::GetRxMsgFDCAN().\n
 * FDCAN init must have been called before and FDCAN filter activation is needed
 * to be able to receive FDCAN messages.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_COM_INIT_NOT_DONE If FDCAN is not initialized
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::StartMsgReceptionFDCAN(void)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint8_t answer[4];
	...
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_START_MSG_RECEPTION_FDCAN;
	pRq->CDBByte[2] = FDCAN_MSG_FORMAT_V2;
	...
	brgStat = SendRequestAndAnalyzeStatus(pRq, (uint16_t*)answer);
	if ((answer[2] != FDCAN_MSG_FORMAT_V2) && (brgStat == BRG_NO_ERR)) { //robustness
		StopMsgReceptionFDCAN();
		brgStat = BRG_PARAM_ERR;
	}
	if (brgStat != BRG_NO_ERR) {
		LogTrace("FDCAN Error (%d) in StartMsgReceptionFDCAN (firmware msg format: %d, host format: %d)",
			(int)brgStat, (int)answer[2], (int)FDCAN_MSG_FORMAT_V2);
	}
...
/**
 * @ingroup FDCAN
 * @brief This routine disables the FDCAN RX messages storage at STLink side.\n
 * FDCAN messages will still be received by the STLink but won't be stored.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_COM_INIT_NOT_DONE If FDCAN is not initialized
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::StopMsgReceptionFDCAN(void)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint16_t status;
	...
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_STOP_MSG_RECEPTION_FDCAN;
...
/**
 * @ingroup FDCAN
 * @brief This routine gets the number of received FDCAN messages available in a given fifo
 * to be retrieved with Brg::GetRxMsgFDCAN().
 * @param[in]   FifoNb  FIFO0 or FIFO1 in witch reading FDCAN Message default is #CAN_MSG_RX_FIFO0.
 *                      To be used in accordance with filter(s) configuration.
 * @param[out]  pMsgNb  Filled with the number of available RX FDCAN messages for the given FIFO at STLink side.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_COM_INIT_NOT_DONE If FDCAN is not initialized
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::GetRxMsgNbFDCAN(uint16_t* pMsgNb, const Brg_CanRxFifoT FifoNb)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint8_t answer[8];
	...
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_GET_NB_RXMSG_FDCAN;
	pRq->CDBByte[2] = (uint8_t)FifoNb;
	...
	brgStat = SendRequestAndAnalyzeStatus(pRq, (uint16_t*)answer);
	*pMsgNb = (uint16_t)(answer[2] | (((uint16_t)answer[3] << 8) & 0xFF00));
	if ((answer[4] != FDCAN_MSG_FORMAT_V2) && (brgStat == BRG_NO_ERR)) { //robustness
		brgStat = BRG_PARAM_ERR;
	}
...
/**
 * @ingroup FDCAN
 * @brief This routine allows to get the available FDCAN messages receieved through the FDCAN interface
 * for a given fifo and stored by the STLink.\n
 * The number of available messages (MsgNb) MUST be retrieved first for the same fifo with Brg::GetRxMsgNbFDCAN().
 * @param[out]  pFdcanMsg pointer on array of at least MsgNb message "header": #Brg_FdcanRxMsgT.
 *                       Each message fields are updated as described in #Brg_FdcanRxMsgT.
 * @param[in]  MsgNb MUST be the value returned by Brg::GetRxMsgNbFDCAN().
 *                    It may be less than the value returned by Brg::GetRxMsgNbFDCAN()
 *                    only if the messages are retrieved with several call to Brg::GetRxMsgFDCAN().\n
 *                    pFdcanMsg must point on a #Brg_FdcanRxMsgT array with at least MsgNb element.\n
 * @warning     If MsgNb > Brg::GetRxMsgNbFDCAN(), firmware will return status error (2bytes) instead of requested
 *              messages this may break the USB com.
 * @param[out]  pBuffer  Data buffer for read data (filled only for data frame with DLC>0).
 * @param[out]  BufSizeInBytes  Is less or equal to real data buffer size in bytes.
 *                              It indicates pBuffer available size for read data.\n
 *                Note: data size that can be received is less or equal to MsgNb*64 (DLC max =64).\n
 *                If BufSizeInBytes is smaller than the sum of all DLC only BufSizeInBytes are copied into pBuffer.
 * @param[out]  pDataSizeInBytes  Filled with the number of data written in pBuffer (in bytes).
 * @param[in]   FifoNb  FIFO0 or FIFO1 in witch reading FDCAN Message default is #CAN_MSG_RX_FIFO0.
 *                      Use the same fifo as the one used for MsgNb (retrieved by Brg::GetRxMsgNbFDCAN()).
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_COM_INIT_NOT_DONE If FDCAN is not initialized
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::GetRxMsgFDCAN(Brg_FdcanRxMsgT* pFdcanMsg, uint16_t MsgNb, uint8_t* pBuffer,
	uint16_t BufSizeInBytes, uint16_t* pDataSizeInBytes, const Brg_CanRxFifoT FifoNb)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint8_t* pAnswer;
	...
	if ((pFdcanMsg == NULL) || (pBuffer == NULL) || (pDataSizeInBytes == NULL) || (MsgNb < 1)) {
		return BRG_PARAM_ERR;
	}

	*pDataSizeInBytes = 0; // Default
	answerSize = MsgNb * FDCAN_READ_MSG_SIZE_V2;
	pAnswer = new uint8_t[answerSize];
	if (pAnswer == NULL) {
		return BRG_MEM_ALLOC_ERR;
	}
	...
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_GET_RXMSG_FDCAN;
	pRq->CDBByte[2] = (uint8_t)MsgNb;
	pRq->CDBByte[3] = (uint8_t)((MsgNb >> 8) & 0xFF);
	pRq->CDBByte[4] = (uint8_t)FifoNb;
	...
	if (brgStat == BRG_NO_ERR) {
		uint8_t overrunErr;
		pReadMsg = &pAnswer[0]; //First received message
		buffDataSize = BufSizeInBytes;
		buffDataOffset = 0;
		for (int j = 0; j < MsgNb; j++) {
			// Fill pFdcanMsg and pBuffer with read data
			// Bytes 0-3: Message ID
			pFdcanMsg[j].Header.ID = (uint32_t)pReadMsg[0] | (((uint32_t)pReadMsg[1]) << 8) |
				(((uint32_t)pReadMsg[2]) << 16) | (((uint32_t)pReadMsg[3]) << 24);
			// byte4 message type
			if ((pReadMsg[4] & 0x1) == 0) { // byte4 bit0 IDE
				pFdcanMsg[j].Header.IDE = CAN_ID_STANDARD;
			} else {
				pFdcanMsg[j].Header.IDE = CAN_ID_EXTENDED;
			}
			// byte4 bit1 RTR (see below, DLC mgt)
			if ((pReadMsg[4] & (0x1 << 2)) == 0) { // byte4 Bit2 ESI
				pFdcanMsg[j].Header.ESI = FDCAN_ESI_ACTIVE;
			} else {
				pFdcanMsg[j].Header.ESI = FDCAN_ESI_PASSIVE;
			}
			if ((pReadMsg[4] & (0x1 << 3)) == 0) { // byte4 Bit3 BRS
				pFdcanMsg[j].Header.BRS = FDCAN_BRS_OFF;
			} else {
				pFdcanMsg[j].Header.BRS = FDCAN_BRS_ON;
			}
			if ((pReadMsg[4] & (0x1 << 4)) == 0) { // byte4 Bit4 FDF
				pFdcanMsg[j].Header.FDF = FDCAN_F_CLASSIC_CAN;
			} else {
				pFdcanMsg[j].Header.FDF = FDCAN_F_FD_CAN;
			}
			// Bit7-5 unused (0)
			// Byte5 DLC
			pFdcanMsg[j].Header.DLC = pReadMsg[5];
			if ((pReadMsg[4] & (0x1 << 1)) == 0) { // byte4 bit1 RTR
				pFdcanMsg[j].Header.RTR = CAN_DATA_FRAME;
				if (buffDataSize >= pFdcanMsg[j].Header.DLC) {
					msgDataSize = pFdcanMsg[j].Header.DLC;
				} else {
					msgDataSize = buffDataSize; // limit copied data to max buffer size
					if (brgStat == BRG_NO_ERR) {
						brgStat = BRG_OVERRUN_ERR;
						LogTrace("FDCAN Data Error in GetRxMsgFDCAN: BufSizeInBytes too small (error at %d/%d msg)",
						          (int)j, (int)MsgNb);
					}
				}
			} else {
				pFdcanMsg[j].Header.RTR = CAN_REMOTE_FRAME;
				msgDataSize = 0; // no data to copy in case of RTR message
			}
			// Byte6-7: Message time stamp
			pFdcanMsg[j].TimeStamp = (uint16_t)pReadMsg[6] | (((uint16_t)pReadMsg[7]) << 8);
			// Byte8: matching filter index
			pFdcanMsg[j].FilterNb = pReadMsg[8];
			// Byte9: Message status (Bit1-0 Overrun, Bit7-2 unused)
			overrunErr = (pReadMsg[9]) & 0x3; // byte9 Bit1-0 Overrun
			if (overrunErr != 0) {
				// Overrun has occurred before this msg
				if (overrunErr == 1) { // CAN fifo overrun err (1)
					pFdcanMsg[j].Overrun = CAN_RX_FIFO_OVERRUN;
				}
				else { // Buffer overrun error (2)
					pFdcanMsg[j].Overrun = CAN_RX_BUFF_OVERRUN;
				}
				if (brgStat == BRG_NO_ERR) {
					brgStat = BRG_OVERRUN_ERR;
					firstErrMsgNb = j;
					LogTrace("FDCAN Overrun Error in GetRxMsgFDCAN (first error %d at %d/%d msg)",
						(int)overrunErr, (int)firstErrMsgNb, (int)MsgNb);
				}
			} else { // Else no overrun error
				pFdcanMsg[j].Overrun = CAN_RX_NO_OVERRUN;
			}
			// Byte 11-10: Unused
			// Byte 12 to 75: 0 to 64 data bytes
			for (int i = 0; i < msgDataSize; i++) {
				pBuffer[buffDataOffset + i] = pReadMsg[FDCAN_READ_MSG_HEADER_SIZE_V2 + i];
			}
			// Point on next message and update the number of remaining data to copy
			pReadMsg += FDCAN_READ_MSG_SIZE_V2;
			buffDataSize -= msgDataSize;
			buffDataOffset += msgDataSize;
		} // End of read Fdcan msg loop
		*pDataSizeInBytes = buffDataOffset;
	}

	if (brgStat != BRG_NO_ERR) {
		LogTrace("FDCAN Error (%d) in GetRxMsgFDCAN (max %d bytes, %d msg)",
			(int)brgStat, (int)BufSizeInBytes, (int)MsgNb);
	}
		...
		if (BufSizeInBytes == 4) { //BufSizeInBytes
			LogTrace("FDCAN R %d msg: 0x%08lx", (int)MsgNb, (unsigned long)*pBuffer);
		}
		else {
			LogTrace("FDCAN R %d msg, %d bytes", (int)MsgNb, (int)BufSizeInBytes);
		}
	}
#endif
...
/**
 * @ingroup FDCAN
 * @brief This routine allows to send a message on FDCAN bus through the FDCAN interface,
 * in the mode initialized by InitFDCAN().\n
 * @param[in]   pFdcanMsg Pointer on a message "header" see #Brg_FdcanMsgT description.
 * @param[in]   pBuffer  Pointer to the data buffer (must be at least size length).
 * @param[in]   SizeInBytes  Number of data bytes to send 0-8, 12, 16, 20, 24, 32, 48 or 64 bytes.
 *                           Max 8 in classic CAN.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_CMD_NOT_SUPPORTED if FDCAN not supported by this STLINK or firmware version
 * @retval #BRG_COM_INIT_NOT_DONE If FDCAN is not initialized
 ...
 * @retval #BRG_NO_ERR If no error
 */
Brg_StatusT Brg::WriteMsgFDCAN(const Brg_FdcanMsgT* pFdcanMsg, const uint8_t* pBuffer, uint8_t SizeInBytes)
{
	STLink_DeviceRequestT* pRq;
	Brg_StatusT brgStat;
	uint8_t msgType, msgDLC;
	...
	if ((pFdcanMsg == NULL) || (pBuffer == NULL)) {
		return BRG_PARAM_ERR;
	}
	// RTR Remote Frame supported only for Classic CAN.
	if ((pFdcanMsg->FDF != FDCAN_F_CLASSIC_CAN) && (pFdcanMsg->RTR == CAN_REMOTE_FRAME)) {
		return BRG_PARAM_ERR;
	}
	// accepted Classic CAN DATA FIELD size: 0-8, 12, 16, 20, 24, 32, 48 or 64 bytes.
	if ((pFdcanMsg->FDF == FDCAN_F_CLASSIC_CAN) && ((pFdcanMsg->DLC > 8) || (SizeInBytes > 8))) {
		return BRG_PARAM_ERR;
	}
	// accepted FD CAN DATA FIELD size: 0-8, 12, 16, 20, 24, 32, 48 or 64 bytes (no RTR in FD CAN: no check on DLC).
	if ((pFdcanMsg->FDF == FDCAN_F_FD_CAN) && 
		((SizeInBytes > 8) && (SizeInBytes != 12) && (SizeInBytes != 16) && (SizeInBytes != 20)
		&& (SizeInBytes != 24) && (SizeInBytes != 32) && (SizeInBytes != 48) && (SizeInBytes != 64))) {
		return BRG_PARAM_ERR;
	}

	msgType = 0; // Default CAN_ID_STANDARD, CAN_DATA_FRAME, ESI_ACTIVE, BRS_OFF, CAN_CLASSIC
	if (pFdcanMsg->IDE == CAN_ID_EXTENDED) {
		msgType |= 1; // bit0
		if (pFdcanMsg->ID > 0x1FFFFFFF) { // 29bits ID
			return BRG_PARAM_ERR;
		}
	} else if (pFdcanMsg->ID > 0x7FF) { // CAN_ID_STANDARD 11bits ID
		return BRG_PARAM_ERR;
	}
	if (pFdcanMsg->RTR == CAN_REMOTE_FRAME) {
		msgType |= 1 << 1; // bit1
		msgDLC = pFdcanMsg->DLC;
	} else {
		msgDLC = SizeInBytes;
	}
	if (pFdcanMsg->ESI == FDCAN_ESI_PASSIVE) {
		msgType |= 1 << 2; // bit2
	}
	if (pFdcanMsg->BRS == FDCAN_BRS_ON) {
		msgType |= 1 << 3; // bit3
	}
	if (pFdcanMsg->FDF == FDCAN_F_FD_CAN) {
		msgType |= 1 << 4; // bit4
	...
	pRq->CDBLength = STLINK_BRIDGE_CMD_SIZE_16;
	pRq->CDBByte[0] = STLINK_BRIDGE_COMMAND;
	pRq->CDBByte[1] = STLINK_BRIDGE_WRITE_MSG_FDCAN;
	// ID
	pRq->CDBByte[2] = (uint8_t)(pFdcanMsg->ID);
	pRq->CDBByte[3] = (uint8_t)(pFdcanMsg->ID >> 8);
	pRq->CDBByte[4] = (uint8_t)(pFdcanMsg->ID >> 16);
	pRq->CDBByte[5] = (uint8_t)(pFdcanMsg->ID >> 24);
	// IDE, RTR, ESI, BRS, FDF
	pRq->CDBByte[6] = msgType;
	pRq->CDBByte[7] = (uint8_t)msgDLC;
	if (SizeInBytes > 0) {
		pRq->BufferLength = SizeInBytes;
		pRq->InputRequest = REQUEST_WRITE_1ST_EPOUT;
		pRq->Buffer = (void*)&pBuffer[0];
	} else {
		// Just send the cmd (no data follows: CAN_REMOTE_FRAME case)
		pRq->BufferLength = 0;
		pRq->InputRequest = REQUEST_READ_1ST_EPIN;
		pRq->Buffer = NULL;
	}
	...
	if (brgStat != BRG_NO_ERR) {
		LogTrace("FDCAN Error (%d) in WriteMsgFDCAN (%d bytes)", (int)brgStat, (int)SizeInBytes);
	}
	return brgStat;
}
 ...
 * @param[out] pVoltage  Target voltage in V.
 *
 * @retval #BRG_NO_STLINK If Brg::OpenStlink() not called before
 * @retval #BRG_PARAM_ERR If NULL pointer
 * @retval #BRG_NO_ERR If no error
...
{
	return ConvSTLinkIfToBrgStatus(StlinkDevice::GetTargetVoltage(pVoltage));
}
...
/**
 * @ingroup I2C
 * @retval false If FW is too old for I2C repeated start support (less than v3B6 or v4B3).
 * @note To be called after Brg::OpenStlink().
 */
bool Brg::IsRestartI2CSupport(void) const {
	// Return false if FW is too old for I2C_FULL_RW_TRANS_WRITE_READ, I2C_PREP_RESTART, I2C_FORCE_RESTART
	// support in STLINK_BRIDGE_READ_I2C/ STLINK_BRIDGE_WRITE_I2C command.
	// Used for repeated start sequence (such as  write - repeated start - read).
	if( ((m_Version.Major_Ver==3) && (m_Version.Bridge_Ver<FIRMWARE_BRIDGE_MIN_VER_FOR_RESTART_I2C)) ||
		((m_Version.Major_Ver==4) && (m_Version.Bridge_Ver<FIRMWARE_BRIDGE_V4_MIN_VER_FOR_RESTART_I2C))) {
		return false;
	} else {
		return true;
	}
...
/**
 * @ingroup CAN
 * @retval false If FW does not support all CAN filter initialization mode.
 * @note To be called after Brg::OpenStlink().
 */
bool Brg::IsCanFilter16Support(void) const {
	// Return false if FW does not support 16bit filter scale and filter with mixed STD and EXTID
	if( (m_Version.Major_Ver==4) ) {
		return false;
	} else {
		return true;
	}
...
/**
 * @ingroup FDCAN
 * @retval false If FW is too old for FDCAN support or if STLINK does not support FDCAN interface.
 * @note To be called after Brg::OpenStlink(). STLINK-V3PWR supports both CAN and FDCAN (exclusively).
 * STLINK-V3 (V3SET, V3MODS...) supports only CAN.
 */
bool Brg::IsFdcanSupport(void) const {
	// Return false for STLINK-V3 (CAN only) and first version of STLINK-V3PWR
	// (older STLINKs have no Bridge interface)
	if ((m_Version.Major_Ver < 4) ||
		((m_Version.Major_Ver == 4) && (m_Version.Bridge_Ver < FIRMWARE_BRIDGE_V4_MIN_VER_FOR_FDCAN))) {
		return false;
	} else {
		return true;
	}
