...
start_component() {
  ...
  if has_systemd_user; then
    systemd-run \
      ...
      "$SCRIPT_DIR/tactile_grasp_studio_component.sh" "$component" "$log_file" >/dev/null
    sleep 1
  ...
  if command -v tmux >/dev/null 2>&1; then
    tmux new-session -d -s "$session" "$SCRIPT_DIR/tactile_grasp_studio_component.sh" "$component" "$log_file"
    sleep 1
    if ! tmux has-session -t "$session" 2>/dev/null; then
  ...
  case "$component" in
    ros)
      setsid "$SCRIPT_DIR/tactile_grasp_studio_component.sh" ros "$log_file" < /dev/null &
      ;;
    frontend)
      setsid "$SCRIPT_DIR/tactile_grasp_studio_component.sh" frontend "$log_file" < /dev/null &
      ;;
    *)
...
echo "[start] stopping stale Tactile Grasp Studio processes"
